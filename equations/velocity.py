...
class Velocity(object):
  ...
  def prediction_step(
    ...
    states_buf = {
        _KEY_U: common_ops.divide(rho_u, rho_mid),
        _KEY_V: common_ops.divide(rho_v, rho_mid),
        _KEY_W: common_ops.divide(rho_w, rho_mid),
    ...
    updated_velocity.update({
        ...
        _KEY_V: v,
        _KEY_W: w,
        _KEY_RHO_U: common_ops.multiply(rho_mid, u),
        _KEY_RHO_V: common_ops.multiply(rho_mid, v),
        _KEY_RHO_W: common_ops.multiply(rho_mid, w),
    })
  ...
  def correction_step(
    ...
    states_new = {
        _KEY_RHO_U: [
            rho_u_ - dt * dp_ / (2 * self._params.dx)
            for rho_u_, dp_ in zip(states[_KEY_RHO_U],
                                   self._kernel_op.apply_kernel_op_x(dp, 'kDx'))
        ],
        _KEY_RHO_V: [
            rho_v_ - dt * dp_ / (2 * self._params.dy)
            for rho_v_, dp_ in zip(states[_KEY_RHO_V],
                                   self._kernel_op.apply_kernel_op_y(dp, 'kDy'))
        ],
        _KEY_RHO_W: [
            rho_w_ - dt * dp_ / (2 * self._params.dz) for rho_w_, dp_ in zip(
                states[_KEY_RHO_W],
                self._kernel_op.apply_kernel_op_z(dp, 'kDz', 'kDzsh'))
        ],
    }

    states_buf = {
        _KEY_U: common_ops.divide(states_new[_KEY_RHO_U], rho_mid),
        _KEY_V: common_ops.divide(states_new[_KEY_RHO_V], rho_mid),
        _KEY_W: common_ops.divide(states_new[_KEY_RHO_W], rho_mid),
        _KEY_RHO: states[_KEY_RHO],
    }
    ...
    for uvw in _KEYS_VELOCITY:
      ...
      if self.monitor.check_key(monitor_key):
        monitor_vars.update({
            monitor_key:
                tf.stack(common_ops.multiply(states_new[uvw], rho_mid)),
        })
