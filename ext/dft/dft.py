...
def gen_group_assignment(
  ...
  The group assignments are used by `tpu_ops.all_to_all` for broadcasting data
...
def gen_source_target_pairs(
  ...
  The source-target pairs are used by `tpu_ops.collective_permute` for sending
...
def _all_to_all_matmul(a: tf.Tensor,
  ...
  Args:
    ...
    split_dimension: An `int` in the interval [0, n) along which a `Tensor` is
      split in `tpu_ops.all_to_all()` with `n` denoting the rank of the
      `Tensor`.
  ...
  def body(slice_idx, a_replica):
    ...
    a_replica += pick * tf.complex(
        tpu_ops.cross_replica_sum(
            tf.real(a_replica_slice), group_assignment_dim),
        tpu_ops.cross_replica_sum(
            tf.imag(a_replica_slice), group_assignment_dim))
...
def _collective_permute_einsum(v: tf.Tensor,
  ...
  def body(iteration_idx, slice_idx, a, a_replica):
    ...
    a = tf.complex(tpu_ops.collective_permute(tf.real(a), source_target_pairs),
                   tpu_ops.collective_permute(tf.imag(a), source_target_pairs))
