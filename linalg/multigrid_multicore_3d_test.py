...
"""Tests for multicore multigrid_3d."""
...
class MultigridMulticoreTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def set_x_b_initial_values(self):
    ...
    nz = self.full_grid_shape[-1]
    self.x_tiles = [self.x[:, :, i] for i in range(nz)]
    self.b_tiles = [self.b[:, :, i] for i in range(nz)]
  ...
  def step_fn(self, state, replicas, replica_id, n_coarse, num_cycles):
    ...
    x0_tiles = multigrid_3d_utils.unstack(state['x0'])
    b_minus_a_xb_tiles = multigrid_3d_utils.unstack(state['b_minus_a_xb'])
    state['x0'] = multigrid_3d_utils.stack(
        mg_cycle(x0_tiles, b_minus_a_xb_tiles))

    return state['x0']

  def run_multigrid_3d_tpu(self, n_coarse, num_cycles):
    def mg_cycle():
  ...
  def run_multigrid_3d_np(self, n_coarse, num_cycles):
    mg_cycle_xb = multigrid_3d.poisson_mg_cycle_fn_for_one_core(
        self.full_grid_shape,
        n_coarse,
        weight=self.weight,
    ...
    def mg_cycle(x):
      x_tiles = multigrid_3d_utils.unstack(x)
      return multigrid_3d_utils.stack(mg_cycle_xb(x_tiles, self.b_tiles))
  ...
  def run_test_compare_1_core_np_to_many_core_tpu(
    ...
    num_cycles = 5
    x_one_core, normalized_residual_norm_one_core, _ = self.run_multigrid_3d_np(
        n_coarse, num_cycles)
    x_many_core, normalized_residual_norm_many_core, _ = (
        self.run_multigrid_3d_tpu(n_coarse, num_cycles))
  ...
  def test_different_coarsest_subgrid_shapes_approx_equal(self):
    ...
    x_333, _, mg_cycle_333 = self.run_multigrid_3d_np(n_coarse=1, num_cycles=5)

    self.init(many_cores_computation_shape=(1, 1, 1),
              boundary_conditions=boundary_conditions,
              coarsest_subgrid_shape=(4, 4, 4))
    x_444, _, mg_cycle_444 = self.run_multigrid_3d_np(n_coarse=1, num_cycles=5)
