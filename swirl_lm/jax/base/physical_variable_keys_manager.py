...
class PhysicalVariableKeysHelper(object):
  ...
  def _parse_key_text_info(self, additional_state_key: str):
  ...
  def _parse_key(self, additional_state_key: str):
    """Parse the key with the pattern specified in `self._key_pattern`.
  ...
  def parse_key(self, additional_state_key: str):
    """Parse the key with the pattern specified in `self._key_pattern`.
...
class BoundaryConditionKeysHelper(PhysicalVariableKeysHelper):
  ...
  def __init__(self):
    super(BoundaryConditionKeysHelper, self).__init__(
        r'bc_(\w+)_([0-2])_([0-1])'
    )
  ...
  ) -> dict[str, halo_exchange.BoundaryConditionsSpec]:
  ...
  def check_boundaries_updates_from_additional_states(
      self,
      additional_states_keys: list[str],
  ):
    ...
    for key in additional_states_keys:
      ...
      logging.info(
          ...
          'overwritten by `additional_states`.',
          boundary_info[0],
          boundary_info[2],
          boundary_info[1],
      )

  def generate_bc_key(self, varname: str, dim: int, face: int) -> str:
...
class SourceKeysHelper(PhysicalVariableKeysHelper):
  ...
  def _parse_key(self, additional_state_key: str) -> str | None:
  ...
  def generate_src_key(self, varname: str) -> str:
